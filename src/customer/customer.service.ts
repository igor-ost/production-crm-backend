...
export class CustomerService {
  constructor(@InjectRepository(CustomerEntity) private readonly zipperRepository: Repository<CustomerEntity>){}
  ...
  async findOne(id: string): Promise<CustomerEntity> {
    const customer = await this.zipperRepository.findOne({where: {
      id: id
    }})
    if(!customer){
