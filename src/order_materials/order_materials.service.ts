...
export class OrderMaterialsService {
    constructor(@InjectRepository(OrderMaterialEntity)
    ...
    async findOne(id: string): Promise<OrderMaterialEntity> {
      const order_materials = await this.orderMaterialRepository.findOne({where: {
        id: id
      }})
      if(!order_materials){
    ...
    async update(id: string, updateDto: UpdateOrderMaterialDto): Promise<OrderMaterialEntity> {
