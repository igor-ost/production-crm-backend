...
import { CreateTemplateItemDto, MaterialType } from './dto/create-template_item.dto';
import { UpdateTemplateItemDto } from './dto/update-template_item.dto';
import { TemplatesService } from 'src/templates/templates.service';

@Injectable()
export class TemplateItemsService {
  constructor(@InjectRepository(TemplateItemEntity)
  ...
  async findOne(id: string): Promise<TemplateItemEntity> {
    const template_item = await this.templateItemsRepository.findOne({where: {
      id: id
    }})
    if(!template_item){
  ...
  async update(id: string, updateTemplateItemDto: UpdateTemplateItemDto): Promise<TemplateItemEntity> {
